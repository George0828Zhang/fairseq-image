...
class Seq2SeqCollater(object):
    ...
    def _collate_frames(self, frames):
        ...
        [(C, H, W), ...]
        """
        channels = frames[0].size(0)
        h_max = max(frame.size(1) for frame in frames)
        w_max = max(frame.size(2) for frame in frames)
        res = frames[0].new(len(frames), channels, h_max, w_max).fill_(0.0)

        for i, v in enumerate(frames):
            res[i, :, :v.size(1), :v.size(2)] = v
    ...
    def collate(self, samples):
        ...
        # sort samples by descending number of width pixels
        frames_lengths = torch.LongTensor([s["source"].size(2) for s in samples])
